---
type: entity
created: 2026-04-30
updated: 2026-05-06
tags:
  - researcher
  - signal-processing
  - dynamic-time-warping
---

# Alois Sontacchi

**Affiliation**: Institute of Electronic Music and Acoustics, University of Music and Performing Arts Graz, Austria
**Role**: Researcher
**Research Focus**: Audio signal processing, virtual sensing for ANC, secondary path interpolation.

## Key Contributions

- Co-authored Obs-TasNet for online estimation of virtual sensing observation filters (ICASSP 2026)
- Co-authored DTW-based secondary path interpolation for local ANC with moving listeners (IEEE OJSP 2026)

## Related Sources

- [[sources/holzmueller-2026-obs-tasnet-virtual-sensing|Holzmüller 2026: Obs-TasNet for Virtual Sensing]]
- [[sources/holzmuller-2026-dtw-secondary-path-anc|Holzmüller & Sontacchi 2026: DTW for Secondary Path Interpolation in ANC]]
