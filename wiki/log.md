...
## [2026-09-05] ingest | Neural Projection Filter Generation for Multi-Reference Active Noise Control (He et al. 2026)

- **Source**: `raw/papers/he-2026-neural-projection-filter-anc/full-text.md` (Zotero: JG8T77NH; MinerU service rejected the PDF after 4 attempts, extracted locally via pymupdf with caption-anchored figure rendering)
- **Authors**: Yiming He, Kai Chen, Haishan Zou, Jiancheng Tao, Xiaojun Qiu
- **Published**: IEEE Signal Processing Letters 2026 (early access), DOI 10.1109/LSP.2026.3728398
- **DOI**: 10.1109/LSP.2026.3728398
...
- **Source**: `raw/papers/zhu-2025-kronecker-superdirective-beamforming/full-text.txt` (Zotero: 3FFIXGTS)
- **Authors**: Yujie Zhu, Kunlong Zhao, Xueqin Luo, Jilu Jin, Gongping Huang (corresponding), Jingdong Chen
- **Published**: Journal of Signal Processing (信号处理), Vol. 41, No. 9, pp. 1478–1493, Sep. 2025 (in Chinese)
- **DOI**: 10.12466/xhcl.2025.09.003
- **Summary**: LR-RSD — extends two-dimensional Kronecker product beamforming to an N-way, rank-P decomposition of the length-M superdirective filter ($M=\prod L_n$); alternating-iteration algorithm with closed-form MVDR subproblem updates and diagonal loading embedded in every subproblem; at $M=64$ ($K=256$), rank $P=2$ cuts stored complex parameters by 62.5% and the largest matrix-inversion dimension by 87.5% versus conventional RSD with matched (or slightly better) SNR/SIR/STOI, validated on ULA/CCA simulations plus anechoic-chamber measurements.
- **Pages created**: `wiki/sources/zhu-2025-kronecker-superdirective-beamforming.md`; `wiki/concepts/superdirective-beamforming.md`; `wiki/concepts/kronecker-product-beamforming.md`; `wiki/entities/yujie-zhu.md`, `wiki/entities/kunlong-zhao.md`, `wiki/entities/xueqin-luo.md`, `wiki/entities/jilu-jin.md`
...
## [2026-09-19] ingest | Design of the Wiener gain in noisy and reverberant environments (Xiang et al. 2025, re-ingest of published version)

- **Source**: `raw/papers/xiang-2025-wiener-gain-reverberant/full-text.md` (Zotero: BMNB6JBP, published version)
- **Authors**: Qian Xiang, Jingdong Chen, Jacob Benesty, Tao Lei, Chao Pan
- **Published**: Applied Acoustics, March 2025 (article 110491)
- **DOI**: 10.1016/j.apacoust.2024.110491
- **Summary**: Re-ingest of the published version (replaces the earlier extraction). New in the published version: PESQ evaluation (Table 2, best in all four T60/SNR conditions), real-classroom experiments with measured RIRs (Section 4.7, Table 3: 12.6×9.5×4 m room, T60≈675 ms — proposed best on SNR gain 14.88 dB, LSD 7.78 dB, DRR 8.98 dB, PESQ 1.74), spectrogram figure, array/setup photos, updated affiliations (Qian Xiang: Fuyang Normal University), CRediT and funding.
- **Pages updated**: [[sources/xiang-2025-wiener-gain-reverberant|source page]] (overwritten with published-version content, new figure embeds); [[concepts/snr-cdr-wiener-gain|SNR–CDR Wiener Gain]] (PESQ + real-world validation in Empirical behavior); [[entities/qian-xiang|Qian Xiang]] (new Fuyang Normal University affiliation); [[synthesis/multi-channel-speech-enhancement|Multi-Channel Speech Enhancement]] (Insight 1 + sources table row: real-world validation strengthens the classical CDR lineage claim).
- **Raw**: re-extracted full text (581 lines) and 60 figures via MinerU (VLM), replacing the previous extraction artifacts.

---

## [2026-09-19] ingest | Pan, Chen & Benesty 2026: Microphone Array Self-Awareness via a Residual Model of the Covariance Matrix

Ingested from Zotero (IEEE TASLPRO 2026, DOI 10.1109/TASLPRO.2026.3653137). Extracted full text and figures to raw/papers/pan-2026-array-self-awareness/ (MinerU pipeline model after vlm failure). Created source page, 2 new concept pages (array-self-awareness, covariance-matrix-residual-model), updated 3 entity pages (Chao Pan, Jingdong Chen, Jacob Benesty) and 7 concept pages (spatial-covariance-matrix, spatial-coherence, wiener-filter, blind-source-separation, independent-low-rank-matrix-analysis, blind-source-extraction, and related). Updated the MCSE synthesis page with the blind-extraction branch's paradigm inversion (define the background, not the target), reverberation-helps finding, and offline-ILRMA a priori combination. Indexes and statistics updated (total 1362). Verified with verify_all (wikilinks, backlinks, mkdocs build --strict).
