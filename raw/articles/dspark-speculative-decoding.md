...
1. **半自回归生成 (Semi-Autoregressive Generation):** 为了解决草稿质量问题, DSpark设计了一种混合架构. 它保留了一个计算量大的**并行主干网络**, 用于一次性快速生成所有草稿token的"骨架"; 然后在此基础上增加了一个轻量级的**串行模块**(Sequential Head), 在生成草稿时按顺序注入局部依赖信息. 这种设计既保留了并行生成的高速度, 又通过引入token间的关联性显著改善了草稿后半部分的质量, 减缓了接受率的衰减.
2. **置信度调度验证 (Confidence-Scheduled Verification):** 为了解决验证效率问题, DSpark引入了一个动态调度机制. 它训练了一个**置信度头(Confidence Head)** 来预测每个草稿词元被接受的概率. 然后, 一个**硬件感知调度器(Hardware-Aware Scheduler)** 会结合这些置信度分数和系统当前的实时负载(如GPU吞吐量)来动态决定每个请求应该验证多长的草稿. 当系统负载低时, 可以验证更长的草稿以追求更高的单用户速度; 当系统负载高时, 则会自动剪裁掉那些低置信度的草稿后缀, 避免在"垃圾"词元上浪费算力, 从而保证整个系统的吞吐量稳定.
...
1. **教授起草**: 教授 () 先写下第一个字, 比如"今天天气...".
2. **学生抢答 (投机)**: 博士生 () 看到开头, 立刻"投机性地"猜出后面可能的内容, "唰唰唰"地生成一串草稿, 比如"**真不错, 阳光明媚, 万里无云**".
3. **教授批改 (验证)**: 教授 () 拿到这份草稿. 他不用自己一个字一个字地想了, 只需要批改就行. 他把学生的草稿和自己脑子里想说的话, 在一次思考中(一次前向传播)进行并行对比.
4. **接受并修正**:
5. **完成一轮**: 这一轮解码, 系统最终输出了"**真不错 有点阴沉**"这三个词. 但请注意, 教授只亲自"思考"了一次(用于并行验证和生成修正词), 却得到了多个词的输出. 相比于他自己一个字一个字地思考三次, 速度大大提升了.
6. **循环往复**: 接下来, 学生会根据新的结尾"…有点阴沉", 再次进行抢答, 开始新一轮的投机.
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPak5IF0m55d8y0FguNt7ibVxSibgmoniattiaMfHnicH4YfWs51zENuZAlEu9XDvGLfqERILTrSu8icOwG9fJXeMQZpfZf0EWWbr116A/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/llFnu58nOPbtHJwvwiauqqQlxRpQUOaFQsib62bbMV5JW99zK2bYGFZGyjiaR743n7xH3KGjwogJgukt2gibiaVCU2O60s07n47g0Xb8JMOXHek4/640?wx_fmt=jpeg&from=appmsg)

1. 草稿模型在第 步生成了 token, 其概率为
2. 目标模型在第 步如果自己生成, 会以概率 生成 token.
3. 我们进行一次"抛硬币"实验. 以概率 接受 token.
...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/llFnu58nOPacSfIfdwsm96MPngpa2cpZ5NYsBGqjgelqNXWloyicJ4TicxHOQfe1YlibB3j7XkH3Ip1tkX7E3jsnqAu2XJVIau0c6cwaMPOicAA/640?wx_fmt=jpeg&from=appmsg)
...
` # ==================== Step 1: Target Model 并行前向传播 ====================
 # verify_input_ids 的结构: [当前token, draft_token_1, ..., draft_token_k]
 # 长度为 draft_token_count + 1（包含当前 token 本身）
 draft_token_count = int(proposal.draft_token_count)
 verify_length = draft_token_count + 1 # +1 是因为包含了当前已接受的 token
 verify_position_ids = position_ids[:, start : start + verify_length]

 # 将候选序列一次性送入 target model
 ...
 # 将 logits 转换为概率分布 p_target(x), shape: [B, verify_length, V]
...
`
 ...
 # proposed_tokens: draft model 提出的 token 序列, shape: [B, draft_token_count]
 # verify_input_ids = [current_token, t1, t2, ..., tk], 取 [1:] 得到 draft tokens
 proposed_tokens = proposal.verify_input_ids[:, 1:]

 # 从 target_probs 中取出对应 draft token 位置的概率
 # target_probs[:, :-1, :] 对应位置 0..k-1 预测位置 1..k 的概率
 # 即 p_target(t_i | prefix) 对每个 draft token t_i
 selected_target_probs = gather_token_probs(
 target_probs[:, :-1, :],
 proposed_tokens,
 )

 # 从 draft_probs 中取出对应 draft token 的概率 q_draft(t_i)
 # clamp_min 防止除零
 ...
 # 计算接受概率: accept_prob = min(1, p_target(x) / q_draft(x))
 # shape: [B, draft_token_count]
 accept_prob = torch.clamp(
 selected_target_probs / selected_draft_probs,
 max=1.0,
 )

 # 对每个位置生成均匀随机数 u ~ U(0,1), 若 u < accept_prob 则接受
 accept_mask = (torch.rand_like(accept_prob) < accept_prob).to(torch.int64)

 # cumprod 实现前缀截断：一旦某位置被拒绝(=0), 后续所有位置都变为 0
 # 这保证了 speculative decoding 的「前缀接受」语义
 accept_prefix_mask = accept_mask.cumprod(dim=1)

 # 被接受的 draft token 总数 = prefix mask 中 1 的个数
 accepted_draft_tokens = int(accept_prefix_mask.sum(dim=1)[0].item())
 else:
 # 没有 draft token 时, 直接跳过验证
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPauMEERwHVjadXicO3iaWUAuI5hFelm9micQiczjZjyNApjtz0MY4C9hqYwPic8PzRvleqGVicfib8GVrGkYj7vulxzykC6nKTAW2fhQs/640?wx_fmt=png&from=appmsg)

但是后面的内容, 篇幅有限, 还是古法挑选几个比较重要的draft model算法写一下吧...

![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPa4E04jlJM9He5pWNnH62Wyj66nwKwXFiahobPN17e8wqEAwxK9U9G5CLgL1tLdcgCmzkLRMgWKniaXicia5edGnQpVJURuiaoQj08M/640?wx_fmt=png&from=appmsg)
...
在2023年《Accelerating Large Language Model Decoding with Speculative Sampling》[4]中提到的DraftModel源于当时的算力问题, Chinchilla是DeepMind的一个700亿参数的语言模型, 但是由于单卡放不下, 采用了16卡并行推理的策略. 然后他们采用了和Target模型同样的分布式部署, 并构建了一个宽而浅的Draft Model

![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPY7EiaJ1FMicibTg53EX7Q8hwNP4QTZHBSeo8rWn2n7CRRbMSl153ib8FRqsicOZYPJxjHp5H391d7Ndm7WClHZMVWibdKSL1J9S6IaI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPb2Wjmpk9G7IrLlyBPyAhYSKU9YkOfaPw3IZFMJAE7aLWP7HGv6eMykvyndw1sdd1GIHnr4DgfRNjmunianhkz5BEsia4avS0CFQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPakTUBD2Y5t29Gqd85jxeibTGbWDCjANHniaBjHribCiaI13RzB1192INuWOagasIHPlXrTsXIPMQM5gYODxI0RLqgghRVsLDGELaI/640?wx_fmt=png&from=appmsg)

SpecInfer的Draft模型没有从头训练一个全新模型, 而是:

1. 直接使用同族预训练小模型, 这些模型使用相同的词表和大量几乎相同的训练数据, 天然具有一定的分布对齐.
2. Boosting微调, 类似集成学习中的 Boosting——后一个模型专门学习前面模型"做错的题". 这样多个 SSM(Small Speculative Model)组合起来的覆盖率远超单个模型.

然后在Speculative Inference阶段, 先为每个SSM生成一颗输出树, 即在每个token 取topk=5种可能生成的树, 然后将其合并.

![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPazN4t49VOY0HcBZvQXySgrkjGlZia7CIdAcE0EpQsIgODqd1vEhs5PLVkYiarr06KvOYqLxEdeicsibs2U2F6B8VyatAKmNvt7db8/640?wx_fmt=png&from=appmsg)

然后针对Tree-based结构, 通过修改mask的矩阵方法, 使得模型可以一次验证多个 sequence.

![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPYzb0NIZc8fLsCk3rX5fE9gkDuRlhLgOpHgrpUzWf0KTUKl6088dfBkVhDAXpUC3Iz51JYibHDWaZaemib1zB8UWuJb3LVSjBib4I/640?wx_fmt=png&from=appmsg)

之后作者参考 Speculative Sampling, 提出 Multi-Step Speculative Sampling 的算法, 并且证明了和直接从target model采样的等价性. 并且采样通过率更高:

![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPaA0BFpVVGuuQLc534QgPpjZyXmIwibWHRg2h0PNwku1HzO5MSxwwjghM73uhFq5UHZKHv6sAgvO41XYLnEaYBr8r5OxjgBKpAI/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPbgCGQTIKgDsqrt7u2NusZSEN9oXIvhdpfC14VZvSYTvsib0uI4A6ia2O49LHBaXHTnQnib5Pg5LAp3vx98hAJic3ibRc2JyOW1GZGc/640?wx_fmt=png&from=appmsg)K个MEDUSA头各自输出top-预测, 然后组合各个头预测构建成一颗树, 并通过tree attention mask在一次前向传播中验证所有候选. 然后验证时, 采用Typical Acceptance的方法. 拒绝采样虽然保证了分布一致性, 但在实践中过于保守. Typical Acceptance通过引入基于熵的自适应阈值, 实现了一个优雅的折中:
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPanjKEgU058U0XqVAdWhJfoNYCicEdmJjGYdUDN8ickgHULctviaahmV2okEWXrt6xvlLMD1oENKD5icVSeGOxtjEhUzB6CEhbmJCs/640?wx_fmt=png&from=appmsg)

##### Draft Model架构

和传统的投机解码方案有一个对比:

![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPbl2FnQzGZaHVqicmYMuHXcn7aiar4OmR2c0RtjwFNZFaSrrUSJmQIMmiaEd79KBevv5fztp9uEuIkKorxpbpqTv5Jeyic0hYUxuak/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPaMOzZZRZ8TqJ2JW6JuLN90bf5Og2EH5coWg8HEp3cDRbDZXTbuteXmVpicGzRS9wjdyfmGdnedPpTfZEX3STmtndGzGqiaplpzY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPbp4JfCh0fqzzVQMUwEFvy2f3Nl0NVzMBCOaNonpHWyIU88dJfMIxjxQ1s1YrUeVbnCe4MXGCibL85ZIChwCQ3MRicGJYxeUgg24/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPaMzrWWYNib2rSIr9CJPUb7YuP4iceCr9FtY3QdOdDVqaGJuPF1UDliabCmYINMkUt6J9UsOU7JOkYA5DdvUMMRDQk8JesQFswIxQ/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPb1ibbfyp7BibkXE0Auppj9IfrHqTQdgb0WrtC206zlJq7GXlYgeY193AtKhnHZaJ7mUfopmUuxOq3qURtBOAsc24qqchKVvia51s/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPZw1pQwEPCobnZuqxvt4AudWX1usZCO51fq8NwAmmvBdOGxxtQpziagNxWex36to6aibkyLvpnzpALPRaNoou0YnS8VhqY6DhXe0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPZJIhqeTvGcmiabPYXyeDtVdrltqByplic7fEEfSmE8JEhBiamaup5kQ4cYaWBb4zdiacaVZUcTXgay8l0JqJicLicEHP4NCc9Z5wjcE/640?wx_fmt=png&from=appmsg)

在EAGLE-3中, 作者认为发现了Scaling Law, 即为草稿模型提供的训练数据越多, LLM的推理加速比就越高, 并且这种性能提升呈现出持续、可预测的增长趋势. 然后稍微懒了一下, 用模型总结了一个表

![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPboMlvdfKZPCjJweey1T2vkmoLtXSXAaY5X7BYspQ1ntqB9xntOaCd189CTUhZCE1ofAeTE15R1ueM3XwNgDHltT0QDYw96cxo/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPaxJHxDht1Xic1XPlWhXFdLoK7KDziaIkxNXIfB41kUNT2tFia8fBCr1icqxUldaTqHgwfQcyKribPbdL8YMQRkhEel4mjH4r6eZW1U/640?wx_fmt=png&from=appmsg)

### 1.4.2 DeepSeek-V3 MTP

Meta MTP使用**并行**的独立输出头预测个额外token; DeepSeek-V3则是**顺序**预测, 且保持每个预测深度的完整因果链.

![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPZzTkcMOawufM1LdC6NOjhr1ljY2ulhaXg1iaDdIdE4SicbF0RrS97JWobUBynDvndCazTRmR8fAuIyg8MLnvMHJ3UMtYmQOoVq0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPafVdkvRlWNlaf5zrbOA0Wwof9T8quoV0umlPicQTByjkvmM97QypiaZwubiaE2ZjTfbPJEXGrNNj2RB9gqpBfSoOFcV2Jic3NplHY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/llFnu58nOPZsKoZ0NYEOoXiaOnbyoIjtTX1ClfGKWUlVaPOG98CL4ia5WbQ78DgyRy6MSgjkib47Et4KricXOSGE11Xt0tKL15J2licNLZBj1dn4/640?wx_fmt=jpeg&from=appmsg)
...
`def build_target_layer_ids(num_target_layers: int, num_draft_layers: int):
 """
 计算需要从目标模型中提取隐藏状态的层索引.
 核心思想: 从目标模型的多个层中"均匀采样", 获取从浅到深的完整语义信息.

 参数:
 num_target_layers: 目标模型的总层数(如Qwen3-8B有32层)
 num_draft_layers: 草稿模型的层数(如5层), 决定提取几层特征

 返回:
 层索引列表, 如 [1, 8, 15, 22, 28](对于32层目标模型、5层草稿模型)
 """
 if num_draft_layers == 1:
 # 只有1层草稿模型时, 取目标模型的中间层
 return [num_target_layers // 2]

 # 起始层索引(跳过第0层, 因为它接近原始嵌入, 信息量有限)
 start = 1
 # 结束层索引(避免最后3层, 这些层可能过于专注于下一token预测)
 end = num_target_layers - 3
 span = end - start

 # 在 [start, end] 区间内均匀分布 num_draft_layers 个采样点
 return [
...
`def extract_context_feature(
 hidden_states: list[torch.Tensor], # 目标模型所有层的隐藏状态列表
 layer_ids: Optional[list[int]], # 需要提取的层索引
) -> torch.Tensor:
 """
 从目标模型的指定层中提取并拼接隐藏状态, 形成上下文特征.

 原理: 将多层隐藏状态在最后一个维度上拼接.
 例如, 每层隐藏维度为 D=2048, 提取5层后拼接为 5D=10240 维的特征向量.
 后续会通过投影层压缩回 D 维.

 注意 offset=1 的设计: hidden_states[0] 是嵌入层输出,
 hidden_states[1] 才是第0层Transformer的输出, 所以需要 +1 偏移.
 """
 offset = 1 # 跳过嵌入层, hidden_states[0]是embed输出
 selected_states = [hidden_states[layer_id + offset] for layer_id in layer_ids]
 # 在隐藏维度(dim=-1)上拼接, 形成 [batch, seq_len, num_layers * hidden_size] 的特征
...
```
`# DFlashDraftModel.forward() 中的特征投影
target_hidden = self.hidden_norm(self.fc(target_hidden))
# self.fc: Linear(5 * hidden_size → hidden_size), 将5D压缩为D
# self.hidden_norm: RMSNorm, 稳定训练和推理
`
```
...
`class Qwen3DFlashAttention(nn.Module):
 ...
 self.is_causal = False # 关键！扩散起草使用双向注意力, 非因果

 # Q/K/V/O 投影层——与标准Transformer相同
 ...
 def forward(
 self,
 hidden_states: torch.Tensor, # 草稿模型的当前隐藏状态 [B, q_len, D]
 target_hidden: torch.Tensor, # 注入的目标模型上下文特征 [B, ctx_len, D]
 ...
 **kwargs: Unpack[FlashAttentionKwargs],
 ...
 # ═══════════════════════════════════════════════════════════════
 # 步骤1: Query仅来自草稿模型的隐藏状态
 # ═══════════════════════════════════════════════════════════════
 q = self.q_proj(hidden_states)
 q = q.view(bsz, q_len, -1, self.head_dim)
 q = self.q_norm(q).transpose(1, 2) # [B, num_heads, q_len, head_dim]

 # ═══════════════════════════════════════════════════════════════
 # 步骤2: Key和Value来自两个来源的拼接
 # - k_ctx/v_ctx: 来自目标模型的上下文特征(注入部分)
 # - k_noise/v_noise: 来自草稿模型自身的隐藏状态
 # ═══════════════════════════════════════════════════════════════
 k_ctx = self.k_proj(target_hidden) # 目标特征 → Key空间
 k_noise = self.k_proj(hidden_states) # 草稿自身 → Key空间
 v_ctx = self.v_proj(target_hidden) # 目标特征 → Value空间
 v_noise = self.v_proj(hidden_states) # 草稿自身 → Value空间

 # ═══════════════════════════════════════════════════════════════
 # 步骤3: 在序列维度上拼接, 这就是"KV注入"的核心操作！
 # Key: [目标特征的Key | 草稿自身的Key]
 # Value: [目标特征的Value | 草稿自身的Value]
 # 这等效于扩展了注意力机制的"知识库"
 # ═══════════════════════════════════════════════════════════════
 k = torch.cat([k_ctx, k_noise], dim=1).view(bsz, ctx_len + q_len, -1, self.head_dim)
 v = torch.cat([v_ctx, v_noise], dim=1).view(bsz, ctx_len + q_len, -1, self.head_dim)
 k = self.k_norm(k).transpose(1, 2) # [B, num_kv_heads, ctx_len+q_len, head_dim]
 v = v.transpose(1, 2)

 # ═══════════════════════════════════════════════════════════════
 # 步骤4: 应用旋转位置编码 (RoPE)
 # 注意: ctx部分的位置编码对应上下文位置, noise部分对应当前位置
 # ═══════════════════════════════════════════════════════════════
 ...
 # ═══════════════════════════════════════════════════════════════
 # 步骤6: 执行注意力计算
 # Q: [B, heads, q_len, d] × K^T: [B, heads, d, ctx_len+q_len]
 # 每个草稿位置都能"看到"目标模型的上下文 + 块内其他位置
 # ═══════════════════════════════════════════════════════════════
...
`自回归起草 (EAGLE-3):
 t=1: [anchor] → token₁
 t=2: [anchor, token₁] → token₂
 t=3: [anchor, token₁, token₂] → token₃
 ...共需 γ 次前向传播

块扩散起草 (DFlash):
 t=1: [anchor, MASK, MASK, ..., MASK] → token₁, token₂, ..., token_γ
 ...仅需 1 次前向传播.
`
```
...
```
`# === 起草阶段 ===
# block_output_ids 初始为 [anchor_token, MASK, MASK, ..., MASK]
block_output_ids = output_ids[:, start : start + block_size].clone()
block_position_ids = position_ids[:, start : start + block_size]

if block_size > 1:
 # 将 block_output_ids(含MASK token)转换为嵌入
 noise_embedding = target.model.embed_tokens(block_output_ids)

 # 草稿模型一次前向传播: 输入是噪声嵌入, 条件是目标模型特征
 draft_logits = target.lm_head(model(
 target_hidden=target_hidden, # 目标模型上下文特征(KV注入的条件)
 noise_embedding=noise_embedding, # [anchor_emb, MASK_emb, ..., MASK_emb]
 position_ids=position_ids[:, past_key_values_draft.get_seq_length(): start + block_size],
 past_key_values=past_key_values_draft,
 use_cache=True,
 is_causal=False, # 块内双向注意力！
 )[:, 1 - block_size :, :]) # 取后 block_size-1 个位置的输出

 # 裁剪草稿KV缓存(防止累积无效缓存)
 past_key_values_draft.crop(start)

 # 采样得到草稿token, 填入 block_output_ids 的第2~最后位置
...
# 目标模型并行处理整个草稿块
output = target(
 block_output_ids, # [anchor, draft₁, draft₂, ..., draft_{γ-1}]
 position_ids=block_position_ids,
 past_key_values=past_key_values_target,
 use_cache=True,
 output_hidden_states=block_size > 1, # 同时提取隐藏状态供下一轮使用
)

# 目标模型在每个位置的预测(posterior)
posterior = sample(output.logits, temperature)

# ═══════════════════════════════════════════════════════════════
# 接受长度计算——这行代码是整个验证逻辑的精华！
# ═══════════════════════════════════════════════════════════════
acceptance_length = (
 block_output_ids[:, 1:] == posterior[:, :-1] # 逐位置比较: 草稿token vs 目标模型预测
).cumprod(dim=1).sum(dim=1)[0].item() # cumprod: 遇到第一个不匹配就截断
`
```

##### 与标准 Speculative Sampling 的区别

![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPbe2j6bBOGft5XSxrspCUKg4xoAhkvuxquBMUetOibM7DDkdDGH8EtFjjg2VOGSNoN3uxJFcrSk2lW3Picicja3jPjRXiczumhk2tE/640?wx_fmt=png&from=appmsg)
...
accept = (draft == target_prediction)
`
```
...
```
`训练序列: [t₁, t₂, t₃, t₄, t₅, t₆, t₇, t₈, t₉, t₁₀, ...]

随机锚点位置: 3, 7
构建的训练块:
 块1: [t₃, MASK, MASK, MASK] → 目标: [t₄, t₅, t₆]
 块2: [t₇, MASK, MASK, MASK] → 目标: [t₈, t₉, t₁₀]
`
```
...
```
`# DFlashDraftModel 中的参数共享设计
self.fc = nn.Linear(len(self.target_layer_ids) * config.hidden_size, config.hidden_size, bias=False)
self.hidden_norm = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
# ↑ 只有这些是可训练参数(加上Transformer层)

# 嵌入层和LM头共享自目标模型(冻结)
# 在推理时:
noise_embedding = target.model.embed_tokens(block_output_ids) # 共享嵌入
draft_logits = target.lm_head(model(...)) # 共享LM头
`
```
...
```
`# dflash.py L70-79: vanilla DFlash的起草——只用了argmax/sample的结果
draft_logits = target.lm_head(model(
...
past_key_values_draft.crop(start)
block_output_ids[:, 1:] = sample(draft_logits) # ← 仅采样一条路径!
`
```
...
```
`# ddtree.py L363-373: DDTree的起草——保留完整draft_logits用于建树
draft_logits = target.lm_head(model(
 target_hidden=target_hidden,
 noise_embedding=noise_embedding,
 ...
...
past_key_values_draft.crop(start)
# 注意: 不在这里采样! draft_logits直接传给build_ddtree_tree
`
```
...
draft_logits = target.lm_head(model(
 ...
 use_cache=True,
 is_causal=False,
)[:, -draft_horizon:, :]) # 取最后draft_horizon个位置的logits
`
```
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPYTRZd0hspd1gV1pBczkzl3qJHHuSUibNtuQMiaOpib9maCiaqaq0JUbNO7KRSW5SSWyicrI6QTwI3mE3X0iaUptRwSib5kY8O4WBsrQI/640?wx_fmt=png&from=appmsg)
...
```
`# ddtree.py L102-114
topk = min(budget, draft_logits.shape[-1]) # K = min(B, |V|)
depth_limit = int(draft_logits.shape[0]) # L = 15

# GPU上做top-K排序, 然后传回CPU做heap操作
logits = draft_logits.float()
top_logits, top_token_ids = torch.topk(logits, k=topk, dim=-1)
log_z = torch.logsumexp(logits, dim=-1, keepdim=True) # log partition function
...
while heap and node_count < budget:
 ...
 # 推入下一个兄弟: 同位置的下一个概率token
 if rank + 1 < topk:
 sibling_logw = logw - top_log_probs_np[depth-1, rank] + top_log_probs_np[depth-1, rank+1]
 heapq.heappush(heap, (-sibling_logw, ..., parent_index, depth, rank+1, sibling_logw))

 # 推入第一个子节点: 下一个位置的最高概率token
 if depth < depth_limit:
 child_logw = logw + top_log_probs_np[depth, 0]
...
1. **兄弟**: 将当前位置的token替换为概率次高的token (同深度探索)
2. **子节点**: 在末尾追加下一位置的最高概率token (加深探索)
...
```
`# ddtree.py L151-159: 构建树注意力的visibility矩阵
current_length = 1 + node_count # 根节点(bonus token) + B个树节点
visibility_np = np.zeros((current_length, current_length), dtype=np.bool_)
visibility_np[0, 0] = True # 根节点只能看到自己

for index in range(1, current_length):
 parent_index = int(parents_np[index])
 # 关键: 每个节点继承其父节点的可见性, 再加上自己
...
```
`# ddtree.py L169-209
def compile_ddtree_tree(root_token_id, start, node_token_ids, node_depths,
 visibility_cpu, past_length, ...):
 current_length = 1 + int(node_token_ids.numel()) # bonus + B个节点

 # 1. 构建input_ids: [bonus_token, node_1, node_2, ..., node_B]
 verify_input_ids[0, 0] = root_token_id
 verify_input_ids[0, 1:current_length].copy_(node_token_ids)

 # 2. 构建position_ids: 按树深度分配位置编码
 verify_position_ids[0, 0] = start # bonus token的绝对位置
 verify_position_ids[0, 1:current_length].copy_(node_depths)
 verify_position_ids[0, 1:current_length].add_(start) # 转为绝对位置

 # 3. 构建attention_mask: tree block用finfo.min填充, 可见位置填0
 tree_block = attention_mask_buffer[..., :current_length, past_length:past_length+current_length]
 tree_block.fill_(torch.finfo(dtype).min) # 默认全部遮蔽
 tree_block.masked_fill_(visibility, 0) # 可见位置设为0(不遮蔽)
`
```
...
```
`# ddtree.py L408-416: 目标模型一次前向传播验证整棵树
output = target(
 verify_input_ids, # [1, 1+B] — bonus token + B个树节点
 position_ids=verify_position_ids, # 按树深度分配
 attention_mask=verify_attention_mask, # ancestor-only mask
 past_key_values=past_key_values_target,
 use_cache=True,
 output_hidden_states=True,
)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPY6NAmTp1jmicmibmoy6ekYjCiapgiczoG3RbiaYktqNeefjl4jyxzBUfaAjsT1ZsOQc9fZVckzzlhlygfmYZ1eIKRI9Xo4TN5rqf1M/640?wx_fmt=png&from=appmsg)

```
`# ddtree.py L212-223: 沿树找最长匹配路径
def follow_verified_tree(child_maps, posterior):
 posterior_tokens = posterior[0].tolist() # 目标模型在每个位置的预测
 accepted_indices = [0] # 从根节点(bonus token)开始
 current_index = 0
 next_token = int(posterior_tokens[current_index])

 # 核心循环: 目标模型选的token是否在当前节点的子节点中?
 ...
 return accepted_indices, next_token # 接受的路径 + 下一轮bonus token
`
```

![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPa4QhAfP1JPJ1yhVUicxzdfiaib7tgFTF8paQibIy010e7icCfhgdFicuMQlH7Eppd1U1ic0zb5LmFurJcIDJfVyVuwefCKptPu4fcaPw/640?wx_fmt=png&from=appmsg)

验证后, KV cache中包含了所有B+1个节点的KV, 但只有被接受路径上的KV需要保留:

```
`# ddtree.py L419-429: 提交阶段
posterior = sample(output.logits, temperature)
...
# 仅保留被接受路径的target_hidden特征(供下一轮起草)
...
```
`# ddtree.py L226-242: in-place KV cache压缩
def _compact_appended_window(cache_tensor, past_length, keep_current_indices):
 current_length = cache_tensor.shape[-2] - past_length
 # 从新增的window中, 仅保留keep_current_indices指定的位置
 kept_tail = cache_tensor.narrow(-2, past_length, current_length) \
 .index_select(-2, keep_current_indices)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPaMM8a8dDL9Wj7yxCWdJ17tpAhGiccTvZhQU4O0xAiaa7g4BSqddCObIrC8mvb1Au6pzS7F5QVviaVTQw2Th63ibar4Vc8yFIjDVYQ/640?wx_fmt=png&from=appmsg)
...
```
`# jetspec/tree/_core/ancestor.py
...
1. 前缀位置(`is_prefix`)对所有树节点全部可见
2. 树内位置通过查询祖先矩阵(`Ancestor`)决定是否可以 attend
3. 非祖先关系的位置被设为`-inf`, softmax 后权重为 0

```
`# jetspec/core/tree_attention_kernel.py - Triton kernel 核心逻辑
@triton.jit
def _tree_attn_fwd(Q, K, V, Out, Ancestor, sm_scale, prefix_len, N, KV_LEN, ...):
 # ...
 ...
 # 关键掩码逻辑: prefix 位置全部可见, tree 节点只看祖先
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/llFnu58nOPbEkB9ib98Xl7APIJmX762p4y1c8NV9I0YOfhDAib9rJkrl8YVvJLmeu03qCQ7uYwR0rUsMPmwY8kljReBngJSFnyIdytsuejo7k/640?wx_fmt=jpeg&from=appmsg)
...
```
`# jetspec/models/draft_head.py - DFlashDraftModel.forward()
def forward(self, position_ids, attention_mask=None, noise_embedding=None,
 ...
 # 步骤2: 5层解码器处理, 每层注入target特征作为KV
 for layer in self.layers:
 hidden_states = layer(
 hidden_states=hidden_states,
 target_hidden=target_hidden, # 注入融合特征
...
```
`# jetspec/models/draft_head.py - extract_context_feature()
def extract_context_feature(hidden_states, layer_ids: list[int]) -> torch.Tensor:
 """沿通道维度拼接选定目标层的隐状态.
 offset=1 因为HF返回embedding输出在index 0. """
 offset = 1
 selected_states = []
 for layer_id in layer_ids:
 selected_states.append(hidden_states[layer_id + offset])
 return torch.cat(selected_states, dim=-1) # (B, T, 5*4096)
`
```
...
```
`# jetspec/models/draft_head.py - Qwen3DFlashAttention.forward()
def forward(self, hidden_states, target_hidden, position_embeddings, attention_mask, ...):
 bsz, q_len = hidden_states.shape[:-1]
 ctx_len = target_hidden.shape[1]

 q = self.q_proj(hidden_states) # Query 来自草稿token
 # Key/Value 同时来自 target_hidden 和 draft hidden_states
 k_ctx = self.k_proj(target_hidden) # 目标特征的 Key
 k_noise = self.k_proj(hidden_states) # 草稿token的 Key
 v_ctx = self.v_proj(target_hidden)
 v_noise = self.v_proj(hidden_states)
 # 拼接: [context_KV | draft_KV]
...
```
`# jetspec/models/draft_head.py - _build_dflash_causal_attention_mask()
def _build_dflash_causal_attention_mask(*, query, key, cached_kv_len, ctx_len):
 ...
 return mask.masked_fill(can_attend.logical_not().unsqueeze(0).unsqueeze(0),
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPbpXO7roQ3She6n0SNO11Lwn7oILThngKKxgJDdJL9EEqqxfHULpaIWZmXVK5QKFLPQKxcPNibtkEA9ibYdASOPjyWv1GZdB1ibz4/640?wx_fmt=png&from=appmsg)

JetSpec 使用**最佳优先搜索 (Best-First Search)** 构建候选树:

```
`# jetspec/tree/baselines/accum_logp.py - _build_from_topk()
def _build_from_topk(root_token, topk_tokens_cpu, topk_logprobs_cpu, budget, device):
 ...
 # 最大堆, 按累积log概率排序(取负数用于最小堆)
 counter = 0
 heap = [(0.0, counter, 0)] # (neg_cum_logprob, order, node_idx)

 while heap and num_nodes < budget:
 neg_cum_lp, _, node_idx = heapq.heappop(heap) # 弹出最高分节点
...
1. **初始化**: 优先队列放入根节点, 评分为 0
2. **循环扩展**: 弹出最高评分(累积 log-prob 最高)的可扩展节点
3. **子节点生成**: 在该节点深度取 Top-W 候选 token, 计算子节点评分 = 父节点评分 + 当前位置 log-prob
4. **终止条件**: 节点数达到预算 或无可扩展节点
5. **输出**: 构建包含 token_ids, parent_indices, depth, ancestor 等的`DraftTree` 结构
...
```
`# jetspec/tree/_core/accept.py - tree_accept()
def tree_accept(tree: DraftTree, target_logits, temperature=0.0):
 """找到最长被接受的根到叶路径. """
 logits = target_logits.reshape(-1, target_logits.shape[-1])
 posterior = _sample_greedy(logits, temperature) # 目标模型的 argmax
 ...
 current = 0
 while True:
 next_token = posterior_tokens[current] # 目标模型在当前节点的预测
 child_idx = tree.child_maps[current].get(next_token) # 找匹配的子节点
 ...
 correction_token = posterior_tokens[current] # 修正token
...
```
`# jetspec/tree/_core/accept.py - gpu_tree_accept()
def gpu_tree_accept(tree_tokens, greedy_targets, parent_indices, depths, max_depth=None):
 """向量化贪心树接受, 完全在GPU上执行."""

 # ═══════════════════════════════════════════════════════
 # 步骤1: 计算每个节点的局部匹配 (全部并行)
 # ═══════════════════════════════════════════════════════
 match[0] = True # 根节点总是匹配
 match[1:] = (valid_parent[1:] & ~overwritten[1:]
 & (tree_tokens[1:] == greedy_targets[safe_parents[1:]]))
 # 含义: 节点i匹配 ⟺ 目标模型在i的父节点处预测的token == 节点i的token

 # ═══════════════════════════════════════════════════════
 # 步骤2: Pointer Jumping — O(log D) 传播前缀匹配信息
 # ═══════════════════════════════════════════════════════
 prefix_match = match.clone() # 初始: 每个节点只知道自己是否局部匹配
 jump = safe_parents.clone() # 初始: 每个节点的跳跃目标 = 父节点

 for _ in range(max(1, max_depth.bit_length())):
 # 核心操作1: 合并——我匹配 且 我跳到的祖先也匹配 → 整段路径匹配
 ...
 best_node = torch.argmax(score) # 深度最大且前缀完全匹配的节点
`
```

![](https://mmbiz.qpic.cn/mmbiz_jpg/llFnu58nOPbnvyPWdWOxm1e7E8s9olIX0Z9QOlpIwre5VO1n0AkWBbZ9CFYmotDibPaWEJP7dmSGD3A3E9ttOYMx1NicsEO3PeSx6aM8GZZy4/640?wx_fmt=jpeg&from=appmsg)

##### 渣注

个人觉得, 对于JetSpec的因果性, 可能更恰当的说法是**“位置感知的受限注意力模式"** 的工程设计.

![](https://mmbiz.qpic.cn/mmbiz_jpg/llFnu58nOPaxEzMLUUGSfNQdANiaKtQnT24qQOL8K13HFrEUgrDwgDys8M5plna3AjUBvJX6pfOViavJzJfO9Hwib1qdGNJTEL1bFCMy0Hd74o/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPYP4dK6f2HmDlbntRTLNIG5JRs8luuINsAf2RVethTwNInLcweborc3icJepVSibdlSgyfTyaV41o089XtDn2EPwWs7suOAicIGk4/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/llFnu58nOPZEIhsRE0VCzlPpibhsTSPnrWvOBYVlhHzUhJzXaZnKBbgH92JM0T8ktylTYd3RNXI1za17LckqdkPDZKaXjnJqiaIk7XW2k9w2U/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPaswoicaZ0rrNCLzWbkCpBicMMYhCse6oao8nibCSicgjUOf5MeMcEPdnRdnNZj08WeveACXibq2miaTR7XlQtXUnuYvz2XEgEOibPticQ/640?wx_fmt=png&from=appmsg)
...
`class VanillaMarkov(nn.Module):
 ...
 # W1: 嵌入查找表 [vocab_size, r]
 self.markov_w1 = nn.Embedding(self.vocab_size, self.markov_rank)
 # W2: logit 投影 [r, vocab_size]
 self.markov_w2 = nn.Linear(self.markov_rank, self.vocab_size, bias=False)

 def compute_step_bias(self, token_ids, hidden_states):
 """计算单步转移偏差 B(x_{k-1}, ·)"""
 # 查找前一个 token 的马尔可夫嵌入
 prev_emb = self.markov_w1(token_ids.long()) # [batch, r]
 # 投影到词汇表空间
 return self.markov_w2(prev_emb) # [batch, vocab_size]
...
`class RNNHead(VanillaMarkov):
 def __init__(self, *, vocab_size, markov_rank, hidden_size):
 super().__init__(vocab_size=vocab_size, markov_rank=markov_rank)
 # 联合投影: [s_{k-1}; W1[x_{k-1}]; h_k] -> [gate; candidate; output]
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPa8Zic3aHdLA53SPdGn1iaiaoKiaGwrRMNYDlNbf4wSDqD4Umtn2QviaqpP27rPdZDicTwdwRgsdv3PeQWpmNCYX7ElAaRicMcAjmo698/640?wx_fmt=png&from=appmsg)
...
if self.enable_confidence_head:
 ...
 input_dim += config.markov_rank # 加入马尔可夫嵌入特征
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPadWdg9MwI0TTvr7bAnChib7zeDn2PjrnTeRVT5iaW1cCCGbsa1vhxmAl7zECNJhRMFdnjbRRHJ3FKEP1SC59GXjYEuz1nxvO1Gw/640?wx_fmt=png&from=appmsg)
...
1. **收集信息**: 收集所有等待处理的请求的草稿, 以及每个草稿 token 的(校准后)存活概率.
2. **全局排序**: 把所有请求的所有草稿 token , 按照"存活概率"从高到低排一个大队.
3. **模拟"加人"**:
4. **寻找峰值**: 如果新的吞吐量比之前高, 就继续"加人". 如果发现吞吐量开始下降了, 说明"人加得太多, 系统开始拥堵了", 就立刻停止, 采用上一步那个能达到最高吞吐量的验证方案.
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPa93M5Bd1sh0hGtqibJOXFNc4ia6jicaxMpVyqPOofByNc4Vwiam5rvRV4vsLX7x6Rj8CFRYOsfPePzbic6jM6wYYkloBibCK6ibECI9Q/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPbksmmImdW7SRESU6cjyWxPGGA395zXIcs9cT6DRlO2K0sJODfKE5ib0t5BPFeO9MDAnYswrLCPbw4YpkeLQibuho21BEyNelyqg/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPbYfzwBETOev0Al8iaNNgzhAazu6mkRVicUvWzZzQmicHViagGF4NELicnjfOjc5f7THUzsd3cNTN62Kdmzn5QAlGeommtVEcVSNZOU/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPZopsKFpYUSFENMgUxf0CPbLwTDTofyShvZf3IGDUPTnqf8RklzpN0QiboLHC3Y2G8lhmPiaJtgt9j19FTkBicLn0icaxq5s6AOTuY/640?wx_fmt=png&from=appmsg)
...
1. **解决了时序冲突**: 决定"How many" () 的计算可以在两个周期前就完成, 完美配合 ZOS 流水线, 没有任何等待和停顿.
2. **意外地保证了算法无损性**: 附录A中证明, 如果你回头看当前步的结果来决定是否截断, 会引入偏差, 破坏原始分布. 但 DSpark 的异步设计, 决定截断长度 时, 看的是两步前的数据, 与当前步生成的具体 token 内容完全无关. 这就像一道"因果屏障", 把决策和结果隔离开了. 这使得作者可以大胆地去掉`early-stopping`, 进行全局搜索来找到真正的吞吐量最大点, 而不必担心破坏数学上的正确性.
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPbFX4DNrNLB8QJxp3zAUus5JRuS7akJgGOahOzHWn2RF3IgcTHiahV0ylZYVrDLDiacGQYgu0kZM9gM3NiaqYsyuicd0JFdGnBOEG0/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/llFnu58nOPYQ1RYp2Jzts9jaAaMia97ylRdgKOC4oz41ZKawc8tKbOpdLtf60fjLTJ0NZvftnzqr9Fnnicxiczqm3UUNkcpYAwk1QQFT2DZGCY/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPZlatRtXOBuXSA56Bms0WvhJN5CmkGbeYBkSPltbytXEtricC9AS0epJK5IFPFo7iaEEHHGZetH4xct7O5EcbS7PPs8bdwU5jRYM/640?wx_fmt=png&from=appmsg)
...
1. **TV 距离是 OT 的特殊情况**: 当度量空间为离散的 时,.
2. **分布对齐的"运输代价"**: 草稿模型训练的本质是**最小化将 运输到 的代价**. 在投机解码中, 这等价于最大化接受率:

1. **逐位置的链式分解**: 在位置, 条件接受率为:
...
1. **模型容量约束**: 草稿模型的参数量/层数直接决定了其表达能力上界
2. **延迟约束**: 必须远小于, 否则投机解码无收益
3. **逐位置非独立**: 位置 的接受率依赖于前缀路径
...
1. **并行扩散起草**: 将草稿模型重新定位为"目标模型的扩散适配器"——输入锚点 token + 个 MASK token, 块内所有位置双向注意力, 单次前向传播一次性输出 个草稿 token
2. **KV 注入条件化**: 在草稿模型的**每一层**将目标模型的多层隐藏状态拼接到 K 和 V 中, 使每层都能直接访问目标信息——解决了 EAGLE 输入端融合的"信息逐层衰减"问题
...
![](https://mmbiz.qpic.cn/mmbiz_png/llFnu58nOPbp8vFqiabpHYC2fAYNYd4UYXTYuibdryZ4diaSiaNmlMSTJ2hEnmn7HdnjWJhiaDMiadojS5QBHLMHrf4cnbyasp0Ekxf6hCW1hZSIs/640?wx_fmt=png&from=appmsg)

**四条主线**:

1. **草稿来源**: 独立模型 → 模型自身(跳层/多头/特征) → 检索/Jacobi → 训练内置 → 扩散适配器 → 半自回归
2. **验证方式**: 单序列拒绝采样 → 树验证 → 最优树(DP) → 扩散树 → 因果树 → 硬件感知自适应调度
3. **起草延迟**: 串行 → 并行 → 并行+因果 → 并行+依赖+自适应
4. **系统集成**: 离线 benchmark → 训练-推理统一 → 异步调度 → 生产流量部署
...
`def forward(
 self,
 input_ids: torch.Tensor, # [bsz, seq_len] 完整训练序列的 token ids
 target_hidden_states: torch.Tensor, # [bsz, seq_len, m*d] 目标模型多层隐藏状态拼接
 loss_mask: torch.Tensor, # [bsz, seq_len] 哪些位置参与损失计算
 target_last_hidden_states: Optional[torch.Tensor] = None, # [bsz, seq_len, d] 目标模型最后一层hidden
) -> DSparkForwardOutput:
`
```
...
`anchor_positions, block_keep_mask = sample_anchor_positions(
 ...
 num_anchors=self.num_anchors, # 默认 512
 device=device,
)
`
```
...
1. 筛选有效候选: 位置 有效当且仅当`loss_mask[i] > 0.5` 且`loss_mask[i+1] > 0.5`(确保锚点及其第一个预测目标都在有效范围内)
2. 对有效位置赋随机值, 无效位置赋 2.0(排到最后)
3. 按随机值排序, 取前`num_anchors` 个
4. 最终排序使锚点按位置递增(便于后续 attention mask 构建)
5. `block_keep_mask`标记哪些块是真实采样到的(vs 填充的 dummy 块)
...
```
`# 初始化: 所有位置填充 mask_token_id
noise_ids = torch.full((bsz, num_blocks * block_size), mask_token_id)
# 每个块的第一个位置替换为锚点 token
noise_ids[batch_idx, block_starts] = anchor_tokens # anchor_tokens = input_ids[anchor_positions]
# 嵌入查找
return embed_tokens(noise_ids) # → [bsz, num_blocks * block_size, hidden_dim]
`
```
...
```
`context_position_ids = torch.arange(seq_len) # [0, 1, ..., seq_len-1]
draft_position_ids = create_position_ids(anchor_positions, self.block_size)
# 对每个锚点: [anchor_pos, anchor_pos+1, ..., anchor_pos+block_size-1]
...
`def dspark_mask_mod(b, h, q_idx, kv_idx):
 ...
 # 规则1: 可以注意到锚点之前的上下文(因果)
 mask_context = (kv_idx < seq_len) & (kv_idx < anchor_pos)
 # 规则2: 可以注意到同一块内的所有草稿 token(双向)
...
`output_hidden = self._forward_backbone(
 ...
 noise_embedding=noise_embedding, # 草稿输入
 target_hidden_states=target_hidden_states, # 目标模型特征(KV注入)
...
`def _forward_backbone(self, *, position_ids, noise_embedding, target_hidden_states, ...):
 hidden_states = noise_embedding
 # KV注入: 将目标模型多层隐藏状态投影到草稿模型维度
 target_hidden_states = self.hidden_norm(self.fc(target_hidden_states))
 # 计算旋转位置编码
 position_embeddings = self.rotary_emb(hidden_states, position_ids)
 # 逐层通过 DSpark Decoder Layer
 for layer in self.layers: # 5层
 hidden_states = layer(
 hidden_states=hidden_states,
 target_hidden_states=target_hidden_states, # 每层都注入
 attention_mask=attention_mask,
 position_embeddings=position_embeddings,
 )
...
```
`k = torch.cat([k_proj(target_hidden_states), k_proj(hidden_states)], dim=1) # [context; draft]
v = torch.cat([v_proj(target_hidden_states), v_proj(hidden_states)], dim=1)
`
```
...
```
`# 每个锚点的预测目标: anchor_pos+1, anchor_pos+2, ..., anchor_pos+block_size
label_indices = anchor_positions.unsqueeze(-1) + torch.arange(1, block_size+1)
target_ids = torch.gather(input_ids, ...) # ground truth token ids

# 构建 teacher-forcing 的前一 token 序列:
# prev_token_ids[k] = 位置 k 的 "前一个 token"
# 对位置0: 前一个是锚点自身
# 对位置k>0: 前一个是 ground truth target_ids[k-1]
...
if self.markov_head is not None:
 ...
 draft_logits,
 token_ids=prev_token_ids, # teacher-forcing 的前一 token
 hidden_states=output_hidden_4d, # 主干隐藏状态(RNN头需要)
 )
`
```
...
```
`# RNNHead.apply_block_logits:
state = torch.zeros(...) # s_0 初始化为零
for k in range(block_size):
 prev_emb = self.get_prev_embeddings(token_ids[..., k]) # W1[x_{k-1}]
 h_k = hidden_states[..., k, :] # 主干隐藏状态
 state, bias = self._rnn_step(state, prev_emb, h_k) # GRU更新+输出偏差
...
`if target_last_hidden_states is not None:
 # 目标模型在位置 anchor_pos, anchor_pos+1, ..., anchor_pos+block_size-1 的预测
 target_pred_indices = (safe_label_indices - 1).clamp(min=0)
 aligned_target_hidden = torch.gather(target_last_hidden_states, ...)
...
```
`eval_mask = target_valid & (target_loss_mask > 0.5) & block_keep_mask
# 累积乘积: 一旦某位置无效, 之后全部屏蔽
...
`return DSparkForwardOutput(
 draft_logits=draft_logits, # [bsz, num_blocks, block_size, vocab] 草稿分布
 target_ids=target_ids, # [bsz, num_blocks, block_size] ground truth
 eval_mask=eval_mask, # [bsz, num_blocks, block_size] 有效前缀掩码
 block_keep_mask=block_keep_mask, # [bsz, num_blocks] 有效块掩码
 confidence_pred=confidence_pred, # [bsz, num_blocks, block_size] 置信度logits
 aligned_target_logits=aligned_target_logits, # [bsz, num_blocks, block_size, vocab] 目标分布
)
`
```
...
```
`def compute_dspark_loss(*, outputs, loss_decay_gamma, ce_loss_alpha,
 l1_loss_alpha, confidence_head_alpha):
 # 位置衰减权重
 positions = torch.arange(block_size, device=device)
 decay_weights = torch.exp(-positions.float() / float(loss_decay_gamma))
 loss_weight_mask = eval_mask.float() * decay_weights

 # 交叉熵损失
 loss_per_token = F.cross_entropy(flat_logits, flat_targets, reduction="none")
 ce_loss = (loss_per_token * flat_weights).sum() / flat_weights.sum()
 ...
 l1_loss = (l1_dist * loss_weight_mask).sum() / loss_weight_mask.sum()

 # 置信度损失
 confidence_targets = accept_rate_3d.detach() # 解析接受率作为监督
